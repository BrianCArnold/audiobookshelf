<template>
  <div
    id="bookshelf"
    ref="wrapper"
    class="w-full max-w-full h-full overflow-y-scroll relative"
  >
    <!-- Cover size widget -->
    <widgets-cover-size-widget
      class="fixed right-4 z-50"
      :style="{ bottom: streamLibraryItem ? '181px' : '16px' }"
    />

    <div
      v-if="loaded && !shelves.length && !search"
      class="w-full flex flex-col items-center justify-center py-12"
    >
      <p class="text-center text-2xl mb-4 py-4">
        {{ libraryName }} Library is empty!
      </p>
      <div v-if="userIsAdminOrUp" class="flex">
        <ui-btn to="/config" color="primary" class="w-52 mr-2"
          >Configure Scanner</ui-btn
        >
        <ui-btn color="success" class="w-52" @click="scan">Scan Library</ui-btn>
      </div>
    </div>
    <div
      v-else-if="loaded && !shelves.length && search"
    ...
    <div v-else-if="isAlternativeBookshelfView" class="w-full mb-24">
      <template v-for="(shelf, index) in shelves">
        <widgets-item-slider
          v-if="shelf.type === 'book' || shelf.type === 'podcast'"
          :shelf-id="shelf.id"
          :key="index + '.'"
          :items="shelf.entities"
          :continue-listening-shelf="
            shelf.id === 'continue-listening' || shelf.id === 'continue-reading'
          "
          :height="232 * sizeMultiplier"
        ...
        <widgets-authors-slider
          v-else-if="shelf.type === 'authors'"
          :key="index + '.'"
          :items="shelf.entities"
          :height="192 * sizeMultiplier"
          class="bookshelf-row pl-8 my-6"
        >
          <p
            class="font-semibold text-gray-100"
        ...
        <widgets-narrators-slider
          v-else-if="shelf.type === 'narrators'"
          :key="index + '.'"
          :items="shelf.entities"
...
export default {
  ...
  computed: {
    ...
    isAlternativeBookshelfView() {
      return (
        this.$store.getters["getHomeBookshelfView"] ===
        this.$constants.BookshelfView.DETAIL
      );
    },
    bookCoverWidth() {
      var coverSize =
        this.$store.getters["user/getUserSetting"]("bookshelfCoverSize");
  ...
  methods: {
    selectEntity({ entity, shiftKey }, shelfIndex) {
      ...
      const entityShelfIndex = shelf.entities.findIndex(
      ...
      if (shiftKey && lastLastItemIndexSelected >= 0) {
        ...
        for (let i = loopStart; i <= loopEnd; i++) {
          ...
          if (thisEntity) {
            const mediaItem = {
              ...
              hasTracks:
                thisEntity.mediaType === "podcast" ||
                thisEntity.media.audioFile ||
                thisEntity.media.numTracks ||
                (thisEntity.media.tracks && thisEntity.media.tracks.length),
            };
            this.$store.commit("globals/setMediaItemSelected", {
              item: mediaItem,
        ...
        const mediaItem = {
          ...
          hasTracks:
            entity.mediaType === "podcast" ||
            entity.media.audioFile ||
            entity.media.numTracks ||
            (entity.media.tracks && entity.media.tracks.length),
        };
    ...
    async init() {
      this.wrapperClientWidth = this.$refs.wrapper
    ...
    async fetchCategories() {
      ...
      {
        console.log(`Rendering Shelves...`);
        const newShelves = []
        for (const results of categoryResults) {
          newShelves.push(...results);
        }
      ...
      for (let i = 0; i < categoryOptions.length; i++){
        ...
        const thisCategoryResults = [];
        categoryResults[i] = thisCategoryResults;
        console.log(`Requesting Shelf Categoy ${category}`);
        categoryPromises.push(this.$axios
            .$get(
              `/api/libraries/${this.currentLibraryId}/personalized?include=rssfeed,numEpisodesIncomplete&shelves=${category}`
            )
            .then((data) => {
              thisCategoryResults.push(...data);
              renderCategories();
            })
            .catch((error) => {
    ...
    scan() {
      this.$store
        .dispatch("libraries/requestLibraryScan", {
          libraryId: this.$store.state.libraries.currentLibraryId,
        })
        .then(() => {
    ...
    userUpdated(user) {
      if (
        ...
        user.seriesHideFromContinueListening.length
      ) {
        this.removeAllSeriesFromContinueSeries(
          user.seriesHideFromContinueListening
        );
      }
      if (user.mediaProgress.length) {
        const mediaProgressToHide = user.mediaProgress.filter(
          (mp) => mp.hideFromContinueListening
        );
        this.removeItemsFromContinueListeningReading(
          mediaProgressToHide,
          "continue-listening"
        );
        this.removeItemsFromContinueListeningReading(
    ...
    libraryItemsAdded(libraryItems) {
      ...
      const recentlyAddedShelf = this.shelves.find(
        (shelf) => shelf.id === "recently-added"
      );
      ...
      for (const libraryItem of libraryItems) {
        if (
          libraryItem.libraryId === this.currentLibraryId &&
          !recentlyAddedShelf.entities.some((ent) => ent.id === libraryItem.id)
        ) {
    ...
    episodeAdded(episodeWithLibraryItem) {
      const isThisLibrary =
        episodeWithLibraryItem.libraryItem?.libraryId === this.currentLibraryId;
    ...
    removeAllSeriesFromContinueSeries(seriesIds) {
      this.shelves.forEach((shelf) => {
        if (shelf.type == "book" && shelf.id == "continue-series") {
          ...
          shelf.entities = shelf.entities.filter((ent) => {
            if (
              ent.media.metadata.series &&
              seriesIds.includes(ent.media.metadata.series.id)
            )
              return false;
            return true;
          });
    ...
    removeItemsFromContinueListeningReading(mediaProgressItems, categoryId) {
      const continueListeningShelf = this.shelves.find(
        (s) => s.id === categoryId
      );
      if (continueListeningShelf) {
        if (continueListeningShelf.type === "book") {
          continueListeningShelf.entities =
            continueListeningShelf.entities.filter((ent) => {
              if (mediaProgressItems.some((mp) => mp.libraryItemId === ent.id))
                return false;
              return true;
            });
          ...
          continueListeningShelf.entities =
            continueListeningShelf.entities.filter((ent) => {
              ...
              if (
                mediaProgressItems.some(
                  (mp) =>
                    mp.libraryItemId === ent.id &&
                    mp.episodeId === ent.recentEpisode.id
                )
              )
                return false;
              return true;
